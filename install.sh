#!/bin/bash
عال
if [ -z "${GOST_PIPE_BOOTSTRAP:-}" ] && [ ! -t 0 ]; then
  tmp="$(mktemp)"
  cat > "$tmp"
  chmod +x "$tmp"
  exec env GOST_PIPE_BOOTSTRAP=1 bash "$tmp" "$@"
fi
...
detect_deb_release() {
  ...
  if [[ -f /etc/os-release ]]; then
    release_version=$(grep "^VERSION_ID" /etc/os-release | cut -d '=' -f2 | tr -d '"' | tr -d '.')
  fi
  if [[ -z "$release_version" ]]; then
    release_version="2204"
  fi
  case "$release_version" in
    2404|2410|240*|24) release_version="2404" ;;
    2204|2210|22*) release_version="2204" ;;
    *) release_version="2204" ;;
  esac
...
install_packages() {
  ...
  local release_dir
  release_dir="$(detect_deb_release)"
  local deb_arch
  deb_arch="$(detect_deb_arch)"
  if [[ -z "$deb_arch" ]]; then
    echo "Unsupported deb architecture for base packages."
    exit 1
  fi
  ...
  for pkg in "$@"; do
    if dpkg -s "$pkg" >/dev/null 2>&1; then
      echo "${pkg} already installed, skipping download."
      continue
    fi
    ...
    if ! download_file "$pkg_url" "$pkg_dest"; then
      echo "Failed to download ${pkg} from mirror: ${pkg_url}"
      exit 1
    fi
    dpkg -i "$pkg_dest"
  done
...
install_gost_binary() {
  ...
  if [[ -z "$arch" ]]; then
    echo $'\e[31mUnsupported architecture for Gost.\e[0m'
    exit 1
  fi
  ...
  if [ "$choice" -eq 1 ]; then
    version="2.12.0"
    if [ "$arch" = "amd64" ]; then
      filename="gost_${version}_linux_amd64v3.tar.gz"
    else
      filename="gost_${version}_linux_arm64.tar.gz"
    fi
    url="${DOWNLOAD_BASE}/gost/v2/${filename}"
    ...
    url="${DOWNLOAD_BASE}/gost/v3/${filename}"
  else
  ...
  local tmp_dir
  tmp_dir=$(mktemp -d)
  if ! download_file "$url" "$tmp_dir/gost.tar.gz"; then
  ...
  if [ ! -f "$tmp_dir/gost" ]; then
    echo $'\e[31mError: Gost binary not found in archive.\e[0m'
    rm -rf "$tmp_dir"
    exit 1
  fi
...
systemctl enable sysctl-custom
    ...
    echo $'\e[36m1. \e[0mGost version 2.12.0 (official)'
    ...
    if ! [[ "$gost_version_choice" =~ ^[12]$ ]]; then
        echo $'\e[31mInvalid choice. Exiting...\e[0m'
        exit
    fi

# Download and install Gost based on user's choice
if [ "$gost_version_choice" -eq 1 ] || [ "$gost_version_choice" -eq 2 ]; then
    if [ "$gost_version_choice" -eq 1 ]; then
        echo $'\e[32mInstalling Gost version 2.12.0, please wait...\e[0m'
    else
        echo $'\e[32mInstalling Gost version 3.0.0, please wait...\e[0m'
    fi
    install_gost_binary "$gost_version_choice"
...
# If option 4 is selected
    ...
    if [ "$update_choice" == "y" ]; then
        ...
        if ! download_file "${MIRROR_BASE%/}/install/gost/install.sh" "/etc/gost/install.sh"; then
            echo $'\e[31mFailed to download update script from mirror.\e[0m'
            exit 1
        fi
...
# If option 7 is selected
    ...
    if ! [[ "$auto_restart_option" =~ ^[12]$ ]]; then
        echo $'\e[31mInvalid choice. Exiting...\e[0m'
        exit
    fi
...
if [ "$choice" -eq 8 ]; then
    ...
    if ! [[ "$auto_clear_cache_option" =~ ^[12]$ ]]; then
        echo $'\e[31mInvalid choice. Exiting...\e[0m'
        exit
    fi
